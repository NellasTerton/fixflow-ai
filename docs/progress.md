...
Этап 10 в работе: контур автоматизаций Make. Серверная часть отложенных
автоматизаций готова и проверена; в Make остаётся починить дублирующую ветку
Router и собрать сценарий расписания.
...
- Удалить ветку Router, которая срабатывает и на `lead.created`, и на
  `booking.created`: история показывает 5 операций вместо 3.
- Добавить ветки `lead.followup_due` и `booking.reminder_due`.
- Дать каждой ветке уникальный `action` в callback.
- Создать сценарий `FixFlow AI — Delayed automations` с расписанием раз в 15
  минут и одним HTTP-модулем.
- Провести production-проверку по плану из `docs/automations.md`.
